...
export default function Imports() {
  ...
  const { data: rules } = useTable('rules', { orderBy: 'priority' })
  const { data: imports, refetch: refetchImports } = useTable('statement_imports', { orderBy: 'created_at', ascending: false })
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      // Fetch existing transactions in the date range for dupe checking
    ...
    try {
      ...
      const { data: batchRow, error: batchErr } = await supabase
        ...
        .insert({
          ...
          committed_count: parsed.filter(p => p._import).length
        })
        .select()
        .single()
      if (batchErr) throw batchErr

      // Insert all approved transactions with the batch id
      const rows = parsed.filter(p => p._import).map(p => ({
        date: p._date,
        amount: p._amount,
        description: p._description,
        raw_description: p.description,
        category_id: p._category || null,
        account_id: p._account || null,
        person_id: p._person || null,
        notes: p._notes || null,
        source: 'import',
      ...
      // Bump rule hit counts
      const hitCounts = {}
      parsed.filter(p => p._import && p._ruleId).forEach(p => {
        hitCounts[p._ruleId] = (hitCounts[p._ruleId] || 0) + 1
      })
      for (const [ruleId, count] of Object.entries(hitCounts)) {
        const rule = rules.find(r => r.id === ruleId)
        if (rule) {
          await supabase.from('rules').update({ hits: (rule.hits || 0) + count }).eq('id', ruleId)
        }
      }

      setBatchSummary({ id: batchRow.id, count: rows.length, total: parsed.length })
      setStep(STEP_DONE)
      refetchImports()
    } catch (e) {
      console.error(e)
  ...
  const handleUndo = async (batchId) => {
    ...
    try {
      ...
      await supabase.from('statement_imports').update({ status: 'discarded' }).eq('id', batchId)
      refetchImports()
    } catch (e) {
