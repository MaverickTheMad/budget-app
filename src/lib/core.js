...
// If an app hasn't been given the Supabase env vars yet (or you're running
// `vite` locally without a .env), `core` is null and every helper below falls
// back to the local theme cache instead of throwing at import time.
export const core =
  url && key ? createClient(url, key, { db: { schema: 'core' } }) : null
...
export async function getPref(personId, key, fallback = null) {
  if (!core || !personId) return fallback
  const { data } = await core
...
export async function addPerson(sortOrder = 0) {
  if (!core) return null
